...
export function draw (x, y, height, width, fill, colorScale) {
  ...
  const legendAxis = d3.select('.legend.axis').attr('transform', 'translate(' + x + ',' + y + ')')
  const scale = d3.scaleLinear().domain(colorScale.domain()).range([height, 0])
  const colorScaling = d3.axisLeft(scale).ticks(7)

  legendAxis.call(colorScaling)
}
